...
def generate_gradcam_raw(image_path, model_path, device):
    """Generate Grad-CAM heatmap using the original image (no segmentation)."""
    ...
    # ----------------------------
    # Use original image (NO segmentation)
    # ----------------------------
    ...
    # Use original image as the base for overlay
    original = cv2.resize(np.array(original_image), (224, 224))
    original_bgr = cv2.cvtColor(original, cv2.COLOR_RGB2BGR)
    superimposed_img = heatmap * 0.4 + original_bgr
    ...
    # ----------------------------
    os.makedirs("outputs", exist_ok=True)
    output_path = "outputs/gradcam_raw_result.png"
    cv2.imwrite(output_path, superimposed_img)

    print("Raw Grad-CAM saved to outputs/gradcam_raw_result.png")
    print(f"Prediction: {pred_label}")
    print(f"Confidence: {confidence*100:.2f}%")
